...
struct SettingsView: View {
    ...
    init() {
        // Инициализируем локальное состояние копией реальных настроек
    ...
    var body: some View {
        ...
        .onAppear {
            self.localSettings = settingsManager.settings
        }
...
private struct DevicesSettingsTab: View {
    ...
    // --- ИСПРАВЛЕНИЕ: Используем ID для отслеживания выделенной строки ---
    @State private var selection: Device.ID?

    var body: some View {
        VStack(spacing: 0) {
            // Список с возможностью выделения
            List(selection: $selection) {
                ForEach($localSettings.devices) { $device in
                    DeviceRowView(device: $device, onEdit: { onEdit(device) })
                        .tag(device.id) // Устанавливаем тэг для выделения
                }
            ...
            HStack {
                ...
                Button(action: deleteSelectedDevice) { Image(systemName: "minus") }
                    .disabled(selection == nil) // Кнопка активна только если что-то выделено
            ...
            VStack(spacing: 24) {
                SettingsGroup(title: "Поведение приложения", icon: "macwindow") {
                    VStack(spacing: 16) {
                        ...
                        SettingsRow(title: "Интервал фоновой проверки", description: "Как часто проверять статус устройств") {
                            HStack(spacing: 8) {
                ...
                SettingsGroup(title: "Обновления", icon: "arrow.down.circle") {
                                   VStack(spacing: 16) {
                                       SettingsRow(
                                           ...
                                           description: "Установленная версия NetPulse"
                                       ) {
                                       ...
                                       SettingsRow(
                                           ...
                                           description: "Проверить наличие новой версии на GitHub"
                                       ) {
                                           Button(action: {
                                               Task {
                ...
                SettingsGroup(title: "Управление конфигурацией", icon: "doc.text") {
                    VStack(spacing: 16) {
                        SettingsRow(title: "Импорт и экспорт", description: "Сохранение и загрузка всех настроек") {
                            HStack(spacing: 8) {
                        ...
                        SettingsRow(title: "Сброс настроек", description: "Восстановить все настройки по умолчанию") {
...
private struct DeviceEditView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            Text(device.name.isEmpty ? "Новое устройство" : "Редактирование: \(device.name)").font(.title2).fontWeight(.bold)
            Form {
                Section(header: Text("Основные параметры").font(.headline)) {
                ...
                Section(header: Text("Иконка").font(.headline)) {
                    // --- ПРОВЕРЕННОЕ ИСПРАВЛЕНИЕ для Picker ---
                    Picker("Иконка:", selection: $device.icon) {
                ...
                Section(header: Text("SSH Команды (опционально)").font(.headline)) {
                    TextField("Команда Wake (например, etherwake)", text: Binding(get: { device.commands.wake ?? "" }, set: { device.commands.wake = $0.isEmpty ? nil : $0 }))
                    TextField("Команда Reboot", text: Binding(get: { device.commands.reboot ?? "" }, set: { device.commands.reboot = $0.isEmpty ? nil : $0 }))
                    TextField("Команда Shutdown", text: Binding(get: { device.commands.shutdown ?? "" }, set: { device.commands.shutdown = $0.isEmpty ? nil : $0 }))
                }
            }
            HStack {
                Button("Отмена", role: .cancel) { dismiss() }; Spacer()
...
private struct SettingsGroup<Content: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 8) {
                Image(systemName: icon).font(.title3).foregroundColor(.accentColor).frame(width: 20)
    ...
    var body: some View {
        SettingsRow(title: title, description: description) {
            HStack(spacing: 8) {
                ...
                if hasBeenEdited {
                    Image(systemName: isValid ? "checkmark.circle.fill" : "xmark.circle.fill")
                        .font(.caption).foregroundColor(isValid ? .green : .red).transition(.scale.animation(.spring()))
                }
            }
        }
    }
...
private struct SshKeyPicker: View {
    ...
    var body: some View {
        SettingsRow(title: "Путь к SSH ключу", description: "Приватный ключ для авторизации на устройствах") {
            HStack(spacing: 8) {
                Text((keyPath as NSString).abbreviatingWithTildeInPath).lineLimit(1).truncationMode(.middle)
